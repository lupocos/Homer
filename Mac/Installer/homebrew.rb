#!/usr/bin/ruby
# This script extracts and installs Homebrew to /usr/local 
...
#ohai "This script will install Homebrew to /usr/local..."
...
Dir.chdir "/usr/local" do
  ...
  # offline installation from archive retrieved from  on 5 Sept 2011
   $ARCHIVE_PATH = File.expand_path("mxcl-homebrew-3545a8d.tar.gz",File.dirname(__FILE__))
  system "/bin/bash -o pipefail -c '/usr/bin/tar xz -mf \"" + $ARCHIVE_PATH + "\" --strip 1'"
end

warn "/usr/local/bin is not in your PATH." unless ENV['PATH'].split(':').include? '/usr/local/bin'

## not necessary for our for our purposes...
#
